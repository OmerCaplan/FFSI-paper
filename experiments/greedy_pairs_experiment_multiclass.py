"""
Greedy Pairs vs Greedy Singles Feature Selection Experiment - Multiclass
...
Also runs greedy-singles on FULL data for comparison.
...
def run_experiment(data_path, n_trials=100, n_sample_features=1000, k=6,
    ...
    # Extract features and label - MULTICLASS
    label_col = 'Original_Stage'
    drop_cols = ['Sample', 'Original_Stage', 'Stage_Binarized']
    feature_cols = [c for c in df.columns if c not in drop_cols]

    X = df[feature_cols].values.astype(np.int8)

    # Encode labels as integers
    label_mapping = {label: i for i, label in enumerate(df[label_col].unique())}
    labels = df[label_col].map(label_mapping).values.astype(np.int8)
    ...
    for r in all_results:
        ...
        r['comparison'] = {
            'cond_entropy_diff': pairs_h - singles_h,  # negative = pairs better
            'pairs_better': bool(pairs_h < singles_h)  # lower H(Y|X) is better
        }
    ...
    if output_dir:
        ...
        results_path = Path(output_dir) / f"greedy_pairs_vs_singles_multiclass_{timestamp}.csv"
...
DATA_PATH = Path("/Users/macbook/Documents/FFSI-paper/data/methylation_data.csv")
OUTPUT_DIR = Path("/Users/macbook/Documents/FFSI-paper/results")

N_TRIALS = 1
N_SAMPLE_FEATURES = 5000
K = 8
BASE_SEED = 42
