...
def run_all_trials(data_path, n_samples_list, n_features_list, seed=None, n_workers=None,
    ...
    if raw_labels.dtype == bool or (raw_labels.dtype == object and str(raw_labels[0]).upper() in ['TRUE', 'FALSE']):
        # Binary TRUE/FALSE
        labels = np.array([1 if str(v).upper() == 'TRUE' else 0 for v in raw_labels])
    elif raw_labels.dtype == object or not np.issubdtype(raw_labels.dtype, np.number):
        # Categorical labels - convert to integers
...
DATA_PATH = PROJECT_ROOT / "data" / "methylation_data.csv"  # <-- UPDATE THIS FILENAME

# Output paths for results (separate folder for methylation multi-class)
OUTPUT_PATH = PROJECT_ROOT / "results" / "methylation_multiclass" / "ffsi_trials_results_100K.csv"
RAW_DIFF_PATH = PROJECT_ROOT / "results" / "methylation_multiclass" / "ffsi_raw_differences_100K.csv"
DETAILED_FFSI_PATH = PROJECT_ROOT / "results" / "methylation_multiclass" / "ffsi_detailed_cases_100K.csv"
...
N_SAMPLES_LIST = [100000]  # Start with 10k to compare with binary
N_FEATURES_LIST = [7, 8, 9, 10]
